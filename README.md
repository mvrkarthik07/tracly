...
2. Create the backend environment file:
   ...
   Set all backend variables from `backend/.env.example`: `DATABASE_URL` should be the Render External Database URL, while `JWT_SECRET`, `CRON_SECRET`, `FRONTEND_ORIGIN`, and `PORT` should be set for this environment. Add a bcrypt-compatible `PASSWORD_HASH`. Generate a hash without putting the plaintext in source control:
...
- `PASSWORD_HASH` is the bcrypt-compatible hash for the one allowed password.
- `FRONTEND_ORIGIN` is the deployed Vercel origin, for example `https://finance.example.com`.

The cron runs at `16:01 UTC` every Sunday, which is `00:01 SGT on Monday`, after the Singapore week has closed, and calls the protected weekly-report endpoint with `X-Cron-Secret`.

## Vercel deployment

Create a Vercel project rooted at `frontend/`. The included `vercel.json` uses `npm run build` and publishes `dist`.

Set `VITE_API_BASE_URL` from `frontend/.env.example` to the public Render backend URL, for example `https://finance-tracker-api.onrender.com`. Because authentication uses cookies, the Render `FRONTEND_ORIGIN` must match the Vercel production origin exactly. Vercel preview URLs are intentionally not allowed by CORS; deploy previews will fail API requests by design until you explicitly choose to allow them.
